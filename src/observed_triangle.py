...
def plot_average_development_curve(triangle, max_dev=9):
    ...
    fig, ax = plt.subplots(figsize=(6, 3), dpi=150)  # Smaller and sharper
    ...
    ax.set_title("Dev Curves: Recent Years vs. Avg", fontsize=12)
    ax.set_xlabel("Dev Year", fontsize=11)
    ax.set_ylabel("Cumulative Paid", fontsize=11)
    ax.tick_params(labelsize=10)
    ax.grid(True, linestyle='--', linewidth=0.4, alpha=0.5)
    ax.legend(title="Policy Year", fontsize=9.5, title_fontsize=9.5, loc='center left', bbox_to_anchor=(1, 0.5))
